...
flowchart TB
    subgraph INPUT["📥 ENTRADA"]
        TEXT["Texto/Conceito/Questão"]
    end

    subgraph SICKN33["🔬 PROCESSAMENTO SICKN33"]
        SEM["CAMADA SEMÂNTICA<br/>• Extração de significados<br/>• Relações conceituais<br/>• Campos semânticos"]

        SYN["CAMADA SINTÁTICA<br/>• Estrutura argumentativa<br/>• Dependências lógicas<br/>• Padrões retóricos"]

        PRAG["CAMADA PRAGMÁTICA<br/>• Contexto de uso<br/>• Intenções comunicativas<br/>• Pressuposições"]
    ...
    subgraph OUTPUT["📤 SAÍDA"]
        MAP["Mapa Conceitual Estruturado"]
        REL["Rede de Relações"]
        GAPS["Lacunas e Ambiguidades Identificadas"]
    end
    ...
    style SICKN33 fill:#E8F5E9
    style SEM fill:#C8E6C9
    style SYN fill:#A5D6A7
    style PRAG fill:#81C784
```
...
flowchart LR
    subgraph QUERY["🔍 CONSULTA"]
        Q["Questão Filosófica"]
    end

    subgraph KDENSE["⚡ K-DENSE-AI"]
        EMB["Embedding<br/>da Query"]
        SEARCH["Busca<br/>Vetorial"]
        RANK["Reranking<br/>Contextual"]
        Q --> EMB --> SEARCH --> RANK
    end

    subgraph CORPUS["📚 BASE DE CONHECIMENTO"]
        PHIL["Corpus Filosófico"]
        SCI["Literatura Científica"]
        HIST["Contexto Histórico"]
    end

    SEARCH <--> CORPUS

    subgraph RESULTS["🎯 RESULTADOS"]
        TOP["Top-K Passagens<br/>Mais Relevantes"]
        CONN["Conexões<br/>Interdisciplinares"]
    end

    RANK --> TOP
    RANK --> CONN

    style KDENSE fill:#E3F2FD
```
...
```mermaid
flowchart TB
    subgraph BERTOPIC["🧠 PIPELINE BERTopic"]
        D["Documentos"]
        E["Embeddings<br/>(SBERT)"]
        U["Redução UMAP"]
        H["Clustering<br/>HDBSCAN"]
        T["Representação<br/>c-TF-IDF"]

        D --> E --> U --> H --> T
    end

    subgraph TOPICS["📊 TÓPICOS DESCOBERTOS"]
        T1["Tópico 1:<br/>Existencialismo"]
        T2["Tópico 2:<br/>Ética Normativa"]
        T3["Tópico 3:<br/>Filosofia da Mente"]
        TN["Tópico N:<br/>..."]
    end
...
```mermaid
flowchart LR
    subgraph LDA["📈 MODELO LDA"]
        DOC["Documento"]
        THETA["θ (Distribuição<br/>de Tópicos)"]
        TOPIC["Tópico k"]
        BETA["β (Distribuição<br/>de Palavras)"]
        WORD["Palavra"]

        DOC --> THETA --> TOPIC --> BETA --> WORD
    end

    subgraph DIRICHLET["Priors Dirichlet"]
        ALPHA["α (Prior<br/>Tópicos)"]
        ETA["η (Prior<br/>Palavras)"]
    end

    ALPHA --> THETA
    ETA --> BETA

    style LDA fill:#FCE4EC
```
...
```mermaid
flowchart TB
    subgraph MERMAID["🎨 TIPOS DE DIAGRAMAS MERMAID"]
        subgraph FLOW["Fluxos"]
            F1["flowchart TB/LR"]
            F2["graph TD"]
        end

        subgraph SEQ["Sequências"]
            S1["sequenceDiagram"]
            S2["Interações temporais"]
        end

        subgraph CLASS["Classes"]
            C1["classDiagram"]
            C2["Relações OO"]
        end

        subgraph STATE["Estados"]
            ST1["stateDiagram-v2"]
            ST2["Transições"]
        end
...
```mermaid
flowchart LR
    T["TESE<br/>Posição Inicial"] --> AT["ANTÍTESE<br/>Negação"]
    AT --> S["SÍNTESE<br/>Superação"]
    S --> T2["Nova TESE<br/>(em nível superior)"]
    T2 --> AT2["Nova ANTÍTESE"]
    AT2 --> S2["Nova SÍNTESE"]
...
```mermaid
mindmap
  root((EUDAIMONIA<br/>Florescimento))
...
stateDiagram-v2
    [*] --> Inconsciência
    Inconsciência --> PréConsciência: Estímulo
    PréConsciência --> Consciência: Atenção
    Consciência --> Autoconsciência: Reflexão
    Autoconsciência --> Consciência: Perda de foco
    Consciência --> PréConsciência: Distração
    Autoconsciência --> MetaConsciência: Contemplação profunda
    MetaConsciência --> [*]: Dissolução do ego
```
...
```mermaid
flowchart TB
    subgraph THEMES["🎨 TEMAS FILOSÓFICOS"]
        subgraph EXISTENCIAL["Tema Existencialista"]
            E_PAL["Paleta: Cinzas, negros,<br/>vermelhos profundos"]
            E_TIP["Tipografia: Serif,<br/>peso médio"]
            E_MOO["Mood: Angústia,<br/>autenticidade, escolha"]
        end

        subgraph ORIENTAL["Tema Oriental"]
            O_PAL["Paleta: Verdes jade,<br/>brancos, dourados sutis"]
            O_TIP["Tipografia: Clean,<br/>espaço negativo"]
            O_MOO["Mood: Serenidade,<br/>fluxo, equilíbrio"]
        end

        subgraph ANALITICO["Tema Analítico"]
            A_PAL["Paleta: Azuis, brancos,<br/>preto puro"]
            A_TIP["Tipografia: Sans-serif,<br/>monospace para lógica"]
            A_MOO["Mood: Clareza,<br/>precisão, rigor"]
        end

        subgraph UBUNTU["Tema Ubuntu"]
            U_PAL["Paleta: Terracota,<br/>ocres, verdes terra"]
            U_TIP["Tipografia: Orgânica,<br/>calorosa"]
            U_MOO["Mood: Comunidade,<br/>conexão, ancestralidade"]
        end
    end

    style EXISTENCIAL fill:#424242,color:#fff
    style ORIENTAL fill:#81C784
    style ANALITICO fill:#64B5F6
    style UBUNTU fill:#BCAAA4
```
...
```mermaid
flowchart TB
    subgraph PRD["📋 ESTRUTURA PRD"]
        H["HEADER<br/>Título, Versão, Data, Autor"]

        subgraph CONTEXTO["1. CONTEXTO"]
            C1["Problema/Oportunidade"]
            C2["Background Filosófico"]
            C3["Stakeholders"]
        end

        subgraph OBJETIVOS["2. OBJETIVOS"]
            O1["Goals Primários"]
            O2["Goals Secundários"]
            O3["Non-Goals"]
        end

        subgraph REQUISITOS["3. REQUISITOS"]
            R1["Funcionais"]
            R2["Não-Funcionais"]
            R3["Restrições"]
        end
        ...
        subgraph DESIGN["4. DESIGN"]
            D1["Arquitetura Conceitual"]
            D2["Fluxos de Dados"]
            D3["Interfaces"]
        end

        subgraph IMPL["5. IMPLEMENTAÇÃO"]
            I1["Fases"]
            I2["Milestones"]
            I3["Riscos"]
        end

        H --> CONTEXTO --> OBJETIVOS --> REQUISITOS --> DESIGN --> IMPL
    end

    style PRD fill:#E8EAF6
```
...
```mermaid
quadrantChart
    title Matriz de Avaliação Arquitetural
...
flowchart LR
    subgraph SEARCH["🔍 BUSCA"]
        Q["Questão de<br/>Pesquisa"]
        KW["Keywords +<br/>Sinônimos"]
        DB["Bases de Dados<br/>• PhilPapers<br/>• JSTOR<br/>• Google Scholar<br/>• Scopus"]
    end

    subgraph FILTER["⚖️ FILTRO"]
        REL["Relevância"]
        QUAL["Qualidade<br/>(H-index, citações)"]
        REC["Recência"]
    end

    subgraph SYNTHESIS["📝 SÍNTESE"]
        EXTRACT["Extração de<br/>Argumentos"]
        COMPARE["Comparação<br/>de Posições"]
        INTEGRATE["Integração<br/>Crítica"]
    end

    Q --> KW --> DB --> REL --> QUAL --> REC --> EXTRACT --> COMPARE --> INTEGRATE
...
```mermaid
flowchart TB
    subgraph FIRECRAWL["🔥 FIRECRAWL PIPELINE"]
        URL["URLs Seed"]
        CRAWL["Crawler<br/>Recursivo"]
        PARSE["Parser<br/>HTML → Estrutura"]
        CLEAN["Limpeza<br/>e Normalização"]
        STORE["Armazenamento<br/>Estruturado"]

        URL --> CRAWL --> PARSE --> CLEAN --> STORE
    end

    subgraph SOURCES["🌐 FONTES"]
        WIKI["Wikipedia"]
        SEP["Stanford Encyclopedia<br/>of Philosophy"]
        IEP["Internet Encyclopedia<br/>of Philosophy"]
        ARXIV["arXiv / PhilSci"]
    end

    SOURCES --> URL

    style FIRECRAWL fill:#FFEBEE
```
